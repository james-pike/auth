...
export default component$(() => {
  ...
  const handleNewImage = $(async (e: Event) => {
    ...
    if (input.files?.[0]) {
      ...
      newForm.image = await fileToBase64(file);
  ...
  const handleEditImage = $(async (e: Event) => {
    ...
    if (input.files?.[0]) {
      ...
      editForm.image = await fileToBase64(file);
  ...
  return (
    <div class="max-w-4xl mx-auto p-6">
      <h1 class="text-3xl font-bold mb-6">Classes ⚡️</h1>
      ...
      {userSignal.value?.user && (
        <>
          ...
          {classes.value.map((c) => (
            <div key={c.id} class="border p-4 rounded mb-4">
              {editingItem.value === c.id ? (
                <div>
                  <input type="text" class="mb-2 w-full" value={editForm.name} onInput$={(e) => editForm.name = (e.target as HTMLInputElement).value} />
                  <input type="text" class="mb-2 w-full" value={editForm.description} onInput$={(e) => editForm.description = (e.target as HTMLInputElement).value} />
                  <input type="text" class="mb-2 w-full" value={editForm.url} onInput$={(e) => editForm.url = (e.target as HTMLInputElement).value} />
                  <input type="file" accept="image/*" class="mb-2 w-full" onChange$={handleEditImage} />
                  {editImagePreview.value && <img src={editImagePreview.value} alt="Preview" class="mb-2 w-32 h-32 object-cover" />}
                  <label>
                    Active:
                    <input type="checkbox" checked={editForm.isActive === 1} onChange$={(e) => editForm.isActive = (e.target as HTMLInputElement).checked ? 1 : 0} />
                  </label>
                  <div class="mt-2 space-x-2">
                    <button class="bg-green-500 text-white px-3 py-1 rounded" onClick$={saveEdit}>Save</button>
                    <button class="bg-gray-400 text-white px-3 py-1 rounded" onClick$={cancelEdit}>Cancel</button>
                  </div>
                </div>
              ) : (
                <div>
                  ...
                  <p>{c.description}</p>
                  {c.image && <img src={c.image} alt={c.name} class="w-32 h-32 object-cover my-2" />}
                  <p>URL: <a href={c.url} target="_blank" class="text-blue-600">{c.url}</a></p>
                  <p>Status: {c.isActive ? 'Active' : 'Inactive'}</p>
                  <div class="mt-2 space-x-2">
                    <button class="bg-yellow-400 px-3 py-1 rounded" onClick$={() => startEdit(c)}>Edit</button>
                    <button class="bg-red-500 px-3 py-1 rounded text-white" onClick$={() => deleteClass(c.id!)}>Delete</button>
                  </div>
                </div>
              )}
            </div>
